...
import protobuf.SensorData.SensorData
  ...
  def main(args: Array[String]): Unit = {
    ...
    val createAndSendRecord: () => ProducerRecord[String, Array[Byte]] = () => {
      val sensorData: SensorData = {
        ...
        SensorData(sensorId=sensorId, timestamp=timestamp, temperature=temperature, humidity=humidity)
      }

      val serializedSensorData = serializeProtobuf(sensorData)
      val record = new ProducerRecord[String, Array[Byte]](topic, serializedSensorData)
      messagesCount += 1
      println(s"Message $messagesCount: $sensorData sent at ${LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))}")
